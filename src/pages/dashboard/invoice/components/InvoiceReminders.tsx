...
export default function InvoiceReminders({ data }: InvoiceRemindersProps) {
  const [reminders, setReminders] = useState<InvoiceReminder[]>(data?.reminders);
      ...
      <div className="flex items-center gap-[1.2rem] flex-wrap w-full">
        {reminders.map((reminder, index) => (
          <button
            ...
            className={cn(
              "flex items-center gap-4 hover:bg-grey-50 border border-grey-100 md:px-[1.6rem] py-[1.2rem] rounded-[2.4rem] transition-all duration-200 ",
              {
          ...
          >
            <span className="text-[1.2rem] md:text-[1.4rem] w-[14rem] xl:w-full font-medium">
              {reminder.text}
            </span>
