...
export default function AllInvoicesTable() {
  ...
  const handleViewInvoice = (invoiceId: string) => {
    ...
    setModalType("INVOICE_DETIALS_MODAL");
  };

  useOutsideClick({
    ref: statusContainerRef as React.RefObject<HTMLElement>,
  ...
  return (
    <div className="bg-white rounded-[2rem] p-[1.4rem] md:p-[2.4rem] space-y-[1.6rem]">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_.6fr_.4fr] items-center w-full xl:w-[60%]">
        ...
        <div className="relative" ref={statusContainerRef}>
          <button
            ...
            className="h-full flex gap-4 items-center justify-between px-[1.6rem] py-[1rem] border border-grey-200 rounded-[3.2rem] text-[1.2rem] text-grey-600 hover:bg-grey-50 transition-colors"
          >
            <span>STATUS</span>
          ...
          <ul
            className={cn(
              "absolute w-[12rem] md:w-[26rem] mt-3 right-0 flex flex-col bg-white rounded-[1.7rem] md:rounded-[2.4rem] border border-grey-100 p-[.8rem] md:p-[1.6rem] opacity-0 invisible translate-x-full transition-all duration-500",
              {
          ...
          >
            <Menu.Button className="text-[1rem] md:text-[1.4rem]">
              Paid
            </Menu.Button>
            <Menu.Button className="text-[1rem] md:text-[1.4rem]">
              Draft
      ...
      <InvoiceModal
        ...
        isOpen={modalType === "INVOICE_DETIALS_MODAL"}
      />
    </div>
  );
}
